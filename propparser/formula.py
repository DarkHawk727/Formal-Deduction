...
class LogicFormula:
    ...
    def _parse_infix(self, tokens):
        ...
        for token in tokens:
            if isinstance(token, Variable):
                ...
                while (
                    ...
                    and operators[-1].precedence() <= token.precedence()
                ):
        ...
        if len(out) != 0:
            raise Exception(
                "There are variables that are not consumed by any operator in the formula."
            )
    ...
    def _add_operator(self, stack, operator, reverse_operands):
        if isinstance(operator, BinaryOperator):
            if len(stack) < 2:
                raise Exception(
                    "Binary operator at position {} doesn't have enough arguments".format(
                        operator.position
                    )
                )
            ...
            if len(stack) < 2:
                raise Exception(
                    "Unary operator at position {} doesn't have an argument".format(
                        operator.position
                    )
                )
